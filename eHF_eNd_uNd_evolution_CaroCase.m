...
N_176Lu_LM_t0 =  R_LuHf_LM_eq(1,1) *  N_177Hf_LM_t0; 
...
N_147Sm_LM_t0 =  R_SmNd_LM_eq(1,1) *  N_144Nd_LM_t0; % R_SmNd_CHUR_t0 *  N_144Nd_LM_t0; 
...
N_146Sm_LM_t0 =  R_Sm142Nd_LM_eq(1,1) *  N_144Nd_LM_t0; % R_Sm142Nd_CHUR_t0 *  N_144Nd_LM_t0;
...
% strain_rate_normal = 3 * 0.08 * V_KmPerGyr / 2900; % in Gyr-1
% alpha = exp(strain_rate_normal * dt); % streching factor, unitless
% 
% factor = ones(size(t));
% factor(1:500) = 0.25;
% factor(2500:nt) = 0.65;
% strain_rate_normal_DM = 3 *factor * 0.08 .* V_KmPerGyr / 2900; % in Gyr-1
% alpha_DM = exp(strain_rate_normal_DM * dt); % streching factor, unitless
% 
% % make the OM mixing scale the same as all other mantle components, except
% % for DM
% V_KmPerGyr_OM = V* 1e4; %   V_OM * 1e4;in unit Km/Gyr, V was in unit cm/yr
...
% % NOTICE: UM have a higher density, and this density contrast acts to reduce the
% % mixing efficiency
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % TWO DIFFERENT ALPHA
V_average = mean(V) * 1e4; % in unit Km/Gyr, V was in unit cm/yr
t_overturn = round(2*2900/V_average/dt); % # of timestep for LM, RCC, DM, and CR to remelt
V_KmPerGyr = V * 1e4; % in unit Km/Gyr, V was in unit cm/yr
strain_rate_normal = 1.8 * 0.08 * V_KmPerGyr / 2900; % in Gyr-1
alpha = exp(strain_rate_normal * dt); % streching factor, unitless
alpha_OM = alpha; % exp(strain_rate_normal_OM * dt); % streching factor, unitless
...
factor(1:500) = 0.25;
factor(2500:nt) = 0.95;
strain_rate_normal_DM = 1.8 *factor * 0.08 .* V_KmPerGyr / 2900; % in Gyr-1
alpha_DM = exp(strain_rate_normal_DM * dt); % streching factor, unitless
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
M_142Nd = 142;
M_146Sm = 146;
F1_tp = 0.10; % 10% melting at present-day
...
[C_Sm142Nd_BSE_max,N_Sm142Nd_dCC_max,N_Sm142Nd_dCC_total_max,N_Sm142Nd_UM_max,N_Sm142Nd_LM_max,N_Sm142Nd_dCC_ini_max,...
    ...
    l_DM_flag_matrix_max,l_CR_flag_matrix_max,l_RCC_flag_matrix_max,F_consumed_max,...
    e142Nd_sample_max] = ...
...
x_u142Nd = data_u142Nd(:,1);
y_u142Nd = data_u142Nd(:,2);
...
figure(14);hold on;
scatter(x_u142Nd,y_u142Nd,sz,'MarkerEdgeColor',[0 .5 .5],'MarkerFaceColor','c',...
    'LineWidth',1.5); 
...
xlabel('Time (Gyr)','Fontsize',12);ylabel('\mu^{142}Nd','Fontsize',12); axis tight; box on;
...
plot(x_38,y_38,'k-','linewidth',2);
plot(x_45,y_45,'k-','linewidth',2);
plot(t,y,'k:','linewidth',2); box on; axis tight;
